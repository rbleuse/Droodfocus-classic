...
function DF:init_cooldown_frame()
	...
	if not frame then
		...
		-- icones
		for i = 1,4 do
			cdicons[i].frame = CreateFrame("FRAME","DF_cooldown_FRAME_"..i,frame)
			cdicons[i].overlay = CreateFrame("FRAME","DF_cooldown_overlay"..i,cdicons[i].frame)
		...
		if (cdstart and cdstart > 0 and cdduration>2) then
			...
			if cooldown_table[index].active then
				...
				cooldown_table[index].active=false
						_, _, texture, _, _, _, _, _, _ = GetSpellInfo(index)
