...
function DF:init_timerbars_frame()
	...
	for i = 1,nbSpells do
		...
		if not frames[i] then
			...
			-- cadre principal
			frames[i].frame = CreateFrame("FRAME","DF_TIMERBAR_FRAME"..i,DF.anchor[3].frame)
			...
			frames[i].background = CreateFrame("StatusBar","DF_TIMERBAR_BACKGROUND"..i,frames[i].frame)
			frames[i].foreground = CreateFrame("StatusBar","DF_TIMERBAR_FOREGROUND"..i,frames[i].frame)
			frames[i].cooldown = CreateFrame("StatusBar","DF_TIMERBAR_cooldown"..i,frames[i].frame)
			frames[i].spark =  CreateFrame("FRAME","DF_TIMERBAR_SPARK"..i,frames[i].foreground)
			
			for te = 1,2 do
				frames[i].texts[te] = frames[i].foreground:CreateFontString("DF_TIMERBAR"..i.."_TEXT"..te,"ARTWORK")
			end
